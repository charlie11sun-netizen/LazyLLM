...
- For move, content_ref identifies the content being moved, while destination_ref and
  ...
  Keep dependent update/move operations in the requested execution order. For consecutive moves
  to the same destination, list sources in their intended final order.
- instruction describes the complete visible result of the operation.
...
Output semantics:
- Each replacement contains an exact old_string and its complete new_string. Replacements execute
  sequentially: old_string and content_ref must match the document after preceding replacements.
  An update followed by a move must move the updated content, including updates to only part of it.
...
- Copy old_string character for character from the supplied Markdown or an earlier replacement's
  result when operations depend on each other, including punctuation,
...
Complete only the remaining plan above against the supplied CURRENT Markdown document.
The earlier operations below have already succeeded. Do not perform them again. Their old/new
text can help identify content whose original locator no longer matches after an update or move.
Completed changes in execution order:
{completed_changes_json}

The last attempt failed with:
{error}

Attempted replacements (rolled back, NONE of these changes remain applied):
{attempted_replacements_json}

Return a StringReplaceSet for the remaining operations, preserving their order and all unaffected
Markdown. Fix the matching error using exact current text and enough context to disambiguate.
If a prior update changed only part of the moved content, move the complete updated content.
Do not return a separate locator response or expand the task into a whole-document rewrite.
'''
